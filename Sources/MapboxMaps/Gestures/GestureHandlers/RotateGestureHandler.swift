...
internal class RotateGestureHandler: GestureHandler {
    ...
    @objc internal func handleRotate(_ rotate: UIRotationGestureRecognizer) {
        ...
        if rotate.state == .began {
            ...
            let changedAngle = initialAngle + Double(rotate.rotation)
            ...
            let finalAngle = (initialAngle + Double(rotate.rotation)) + (Double(rotate.velocity) * Double(decelerationRate) * 0.1)
